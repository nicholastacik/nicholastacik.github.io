  ...
  <script>
    ...
    (function () {
      ...
      function eligibleClues(clusterId) {
        ...
        return { clues: broad, scoped: false, fellBack: false, broad };
      }

      function rollSampleClue(clusterId) {
        const card = document.getElementById('sample-card');
        if (!card) return;
        const { clues, scoped, fellBack, broad } = eligibleClues(clusterId);
        if (!clues.length) {
          const msg = fellBack
        ...
        // "Another" should always show a *different* clue. If the current (possibly
        // entity-scoped) pool has nothing other than the last clue shown, broaden to the
        // whole category so the button always does something.
        let choices = clues.filter(c => c.clue !== lastSampleClue);
        let broadened = false;
        if (!choices.length) {
          const more = broad.filter(c => c.clue !== lastSampleClue);
          if (more.length) { choices = more; broadened = true; }
          else { choices = clues; }
        }
        const pick = choices[Math.floor(Math.random() * choices.length)];
        lastSampleClue = pick.clue;
        const scopeLabel = (broadened && selectedEntity)
          ? 'Only one clue for &ldquo;' + escapeHtml(selectedEntity.phrase) + '&rdquo; in this window &mdash; showing another from this category'
          : (scoped
